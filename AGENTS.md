...
## 13. Phase 完成后的版本同步（强制）

1. 每个 Phase 在状态从 `test_passed` 进入 `done` 前，必须先执行“更新到最新版”动作，确保本地源码、运行容器与 `http://localhost:5173/` 在线产物三者一致。
2. 若任一环境未命中最新特征，必须先完成同步（重启容器/重建依赖/清缓存等）并复核，再允许进入下一 Phase。
3. 每个 Phase 收尾时，必须在 `PROGRESS.md` 的 `verification` 中记录本次“最新版同步”证据（至少包含在线产物特征或容器内文件特征）。
4. 向用户发出复测指令时，必须明确要求“关闭旧标签页后重新打开页面”再验证，不得仅要求普通刷新。
